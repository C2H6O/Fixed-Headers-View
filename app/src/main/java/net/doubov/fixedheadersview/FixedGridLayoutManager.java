...
public class FixedGridLayoutManager
    ...
    private int mFirstDataWindowPosition;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        View topLeftChild = getChildAt(0);
        if (topLeftChild != null) {
            state.topLeftChildOffsetX = getDecoratedLeft(topLeftChild);
            state.topLeftChildOffsetY = getDecoratedTop(topLeftChild);
            state.firstPosition = mFirstDataWindowPosition;
        }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof SavedState) {
            ...
            if (DEBUG) {
                Log.d(TAG, "mPendingSavedState = " + mPendingSavedState);
            }
        }
    }
    ...
    static class SavedState implements Parcelable {
        ...
        int topLeftChildOffsetX;
        int topLeftChildOffsetY;
        int firstPosition;
        float childsPerContainerWidth;
        ...
        SavedState(Parcel in) {
            this.topLeftChildOffsetX = in.readInt();
            this.topLeftChildOffsetY = in.readInt();
            this.firstPosition = in.readInt();
            this.childsPerContainerWidth = in.readFloat();
        }

        public SavedState(SavedState other) {
            this.topLeftChildOffsetX = other.topLeftChildOffsetX;
            this.topLeftChildOffsetY = other.topLeftChildOffsetY;
            this.firstPosition = other.firstPosition;
            this.childsPerContainerWidth = other.childsPerContainerWidth;
        }
        ...
        public void writeToParcel(Parcel dest, int flags) {
            dest.writeInt(this.topLeftChildOffsetX);
            dest.writeInt(this.topLeftChildOffsetY);
            dest.writeInt(this.firstPosition);
            dest.writeFloat(this.childsPerContainerWidth);
        }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (getChildCount() == 0) { //First or empty layout
            ...
            View remove = recycler.getViewForPosition(0);
            addView(remove);
            measureChildWithMargins(remove, 0, 0);
             ...
             * won't change.
             */
            mDecoratedChildWidth = getDecoratedMeasuredWidth(remove);
            mDecoratedChildHeight = getDecoratedMeasuredHeight(remove);
        ...
        int childLeft;
        int childTop;
        if (!state.isPreLayout() && mPendingSavedState != null) {
            // restore the scroll/offset state
            mFirstDataWindowPosition = mPendingSavedState.firstPosition;
            childLeft = mPendingSavedState.topLeftChildOffsetX;
            childTop = mPendingSavedState.topLeftChildOffsetY;
            mPendingSavedState = null;

            // if, starting with the anchor position, there isn't enough data to fill the viewport
            // width-wise, we should layout out items from right to left. The following code
            // accomplishesh this task in essence.
            int C = getGlobalColumnOfPosition(mFirstDataWindowPosition);
            int Nc = getTotalColumnCount();
            int Vc = mVisibleColumnCount;

            float viewPortWidthTaken = childLeft + (Nc - C) * mDecoratedChildWidth;

            if (viewPortWidthTaken < getHorizontalSpace()) {
                mFirstDataWindowPosition = (mFirstDataWindowPosition - mFirstDataWindowPosition % Nc) + Nc - Vc;
                C = getGlobalColumnOfPosition(mFirstDataWindowPosition);
                childLeft = getHorizontalSpace() - Vc * mDecoratedChildWidth;
                if (DEBUG) {
                    Log.d(TAG, "mFirstPosition: " + mFirstDataWindowPosition + "/" + childLeft);
                }
            }

            // if, starting with the anchor position, there isn't enough data to fill the viewport
            // height-wise, we should layout out items from bottom to top. The following code
            // accomplishesh this task in essence.
            int R = getGlobalRowOfPosition(mFirstDataWindowPosition);
            int Nr = getTotalRowCount();
            int Vr = mVisibleRowCount;

            float viewPortHeightTaken = childTop + (Nr - R) * mDecoratedChildHeight;

            if (viewPortHeightTaken < getVerticalSpace()) {
                mFirstDataWindowPosition = (Nr * Nc + C) - Vr * Nc;
                childTop = getVerticalSpace() - Vr * mDecoratedChildHeight;
            }

            C = getGlobalColumnOfPosition(mFirstDataWindowPosition);
            R = getGlobalRowOfPosition(mFirstDataWindowPosition);

            if (C > Nc - Vc) {
                if (DEBUG) {
                    Log.d(TAG, "Adjusting first position column...");
                }
                mFirstDataWindowPosition--;
                childLeft -= mDecoratedChildWidth;
                if (DEBUG) {
                    Log.d(TAG, "" + mFirstDataWindowPosition + "/" + childLeft);
                }
            }

            if (R > Nr - Vr) {
                if (DEBUG) {
                    Log.d(TAG, "Adjusting first position row...");
                }
                mFirstDataWindowPosition -= Nc;
                childTop -= mDecoratedChildHeight;
                if (DEBUG) {
                    Log.d(TAG, "" + mFirstDataWindowPosition + "/" + childTop);
                }
            }

//            mPendingSavedState = null; // we are done with it
...
//        }
            ...
            if (!state.isPreLayout() && getVerticalSpace() > (getTotalRowCount() * mDecoratedChildHeight)) {
                mFirstDataWindowPosition = mFirstDataWindowPosition % getTotalColumnCount();
                childTop = 0;

                //If the shift overscrolls the column max, back it off
                if ((mFirstDataWindowPosition + mVisibleColumnCount) > state.getItemCount()) {
                    mFirstDataWindowPosition = Math.max(state.getItemCount() - mVisibleColumnCount, 0);
                    childLeft = 0;
                }
            }
            ...
            if (isOutOfRowBounds || isOutOfColBounds) {
                ...
                mFirstDataWindowPosition = firstRow * getTotalColumnCount() + firstCol;
        ...
        if (DEBUG) {
            Log.d(TAG, "onLayoutChildren FILL GRID: " + state.isPreLayout());
        }
    ...
    private void fillGrid(int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        if (DEBUG) {
            Log.d(TAG, "fillGrid: 0 left, 0 top");
        }
    ...
    private void fillGrid(int direction, int emptyLeft, int emptyTop,
        ...
        if (mFirstDataWindowPosition >= getItemCount()) {
            mFirstDataWindowPosition = (getItemCount() - 1);
        }
        ...
        switch (direction) {
            ...
            case DIRECTION_END:
                ...
                break;
            case DIRECTION_UP:
                mFirstDataWindowPosition -= getTotalColumnCount();
                break;
            case DIRECTION_DOWN:
                mFirstDataWindowPosition += getTotalColumnCount();
                break;
    ...
    public void scrollToPosition(int position) {
        ...
        mFirstDataWindowPosition = position;
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (dy > 0) { // Contents are scrolling up
            ...
            if (bottomBoundReached) {
                if (DEBUG) {
                    Log.d(TAG, "bottom bound reached!");
                }
                ...
                if (DEBUG) {
                    Log.d(TAG, "bottonBoundReached delta: " + delta);
                }
            } else {
...
//            delta = -dy;
        ...
        if (bottomBoundReached && (bottomView.getBottom() == getVerticalSpace()) && delta > 0) {
            if (DEBUG) {
                Log.d(TAG,
                      "bottomView.getBottom() + getVerticalSpace(): " + bottomView.getBottom() + "/" + getVerticalSpace());
            }
            return 0;

        }

        if (topBoundReached && (topView.getTop() == 0) && delta < 0) {
            if (DEBUG) {
                Log.d(TAG, "topView.getTop(): " + topView.getTop());
            }
            return 0;
        }
    ...
    private int positionOfIndex(int childIndex) {
        ...
        return mFirstDataWindowPosition + (row * getTotalColumnCount()) + column;
    }
    ...
    private int getFirstVisibleColumn() {
        return (mFirstDataWindowPosition % getTotalColumnCount());
    }
